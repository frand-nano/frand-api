...
* **설정 관리:** `.env` (기본), `.env.test` (테스트용) 파일과 `api/src/config.rs` 모듈 사용
  ...
  * Rocket managed state를 활용하여 핸들러에서 설정값 접근
  * 주요 환경 변수:
...
* **데이터 형식:** JSON (요청/응답)
* **API 응답 구조:** `api/src/response.rs` 에 정의된 표준 구조 사용
...
* **오류 처리:** Rocket catcher 를 구현하여 프레임워크 기본 오류(404, 405 등)를 `ApiError` 형식으로 변환하여 응답 일관성 유지.
* **초기 구현 목표 (MVP):**
...
* **테스트:**
  ...
  * 초기 테스트 시나리오:
    ...
    * `POST /`: `ApiError` (코드: `NOT_FOUND` 또는 `METHOD_NOT_ALLOWED`, 상태 코드: 404 또는 405) - Catcher 구현 후 확인
    * `GET /api/v1/health`: 200 OK, 본문 `{"success":true,"data":{"status":"ok"}}`, Content-Type `application/json`
    * `POST /api/v1/health`: `ApiError` (코드: `NOT_FOUND` 또는 `METHOD_NOT_ALLOWED`, 상태 코드: 404 또는 405) - Catcher 구현 후 확인
  * 단위 테스트는 필요시 추가 (`api/src/` 내 모듈별 `tests` 서브모듈)
  * 테스트 전략 및 커버리지 목표: 향후 결정
* **프로젝트 설정:**
    * 워크스페이스 루트 `Cargo.toml`에서 공통 메타데이터 정의
...
│   │   ├── error.rs    # ApiError 정의 및 Catcher 구현 모듈 (response.rs 에서 분리 제안)
│   │   ├── handlers/   # 요청 핸들러 모듈
│   │   │   ├── mod.rs
...
* **`api/src/lib.rs`**: `api` 패키지의 라이브러리 코드. Rocket 인스턴스 생성 및 설정 함수 (`build_rocket`) 포함. 핸들러, 라우트, catcher, 상태 관리 설정.
* **`api/src/config.rs`**: `Config` 구조체 정의. 환경 변수 로딩 및 `Config` 인스턴스 생성 함수 (`load_config`) 구현.
    * 의존성: `std::env`, `dotenvy`.
* **`api/src/logger.rs`**: 로거 초기화 함수 (`init_logger`) 구현. `LOG_LEVEL` 환경 변수 기반 로깅 레벨 설정.
    * 의존성: `log`, `simple_logger`, `std::env`.
* **`api/src/response.rs`**: `ApiResponse<T>` 구조체 및 관련 구현 정의. `serde::Serialize` 활용.
    * 의존성: `serde`, `rocket`.
* **`api/src/error.rs`**: `ApiError` 구조체 및 `Responder` 구현 정의. 표준 오류 코드 상수 정의. Rocket catcher 함수들 (`not_found`, `internal_error` 등) 구현.
...
* **`api/src/handlers/root.rs`**: `/` 경로의 `GET` 요청 처리 핸들러 (`root_handler`) 구현. `text/plain` 응답 반환.
    * 의존성: `rocket`.
* **`api/src/handlers/health.rs`**: `/api/v1/health` 경로의 `GET` 요청 처리 핸들러 (`health_check_handler`) 구현. `ApiResponse<HealthStatus>` 반환.
...
| **함수** | `load_config`        | Function     | 환경 변수 로드 및 `Config` 인스턴스 생성                             | `()` -> `Result<Config, Error>`                                                                                                                     | `api/src/config.rs`   |
|              | `init_logger`        | Function     | 로거 초기화                                                          | `(log_level: &str)` -> `Result<(), SetLoggerError>`                                                                                                  | `api/src/logger.rs`   |
|              | `build_rocket`       | Async Function | Rocket 인스턴스 생성, 설정, 라우팅, catcher, 상태 관리 설정        | `(config: Config)` -> `Result<Rocket<Build>, Error>`                                                                                                | `api/src/lib.rs`      |
|              | `root_handler`       | Async Function | `/` 경로 GET 요청 핸들러                                             | `()` -> `&'static str` (반환값: "hello world")                                                                                                      | `api/src/handlers/root.rs` |
|              | `health_check_handler` | Async Function | `/api/v1/health` 경로 GET 요청 핸들러                            | `()` -> `Result<Json<ApiResponse<HealthStatus>>, ApiError>`                                                                                         | `api/src/handlers/health.rs` |
|              | `not_found`          | Function     | 404 오류 Catcher                                                     | `(&Request)` -> `Json<ApiError>`                                                                                                                    | `api/src/error.rs`    |
|              | `internal_error`     | Function     | 500 오류 Catcher (필요시 구현)                                       | `(&Request)` -> `Json<ApiError>`                                                                                                                    | `api/src/error.rs`    |
...
* Rocket 의 `#[catch]` 매크로를 사용하여 기본 오류 핸들러(404 Not Found, 405 Method Not Allowed 등)를 등록.
* Catcher 함수는 해당 HTTP 상태 코드에 맞는 `ApiError` 를 생성하여 반환.
    ...
    * 500 -> `code: INTERNAL_SERVER_ERROR`, `status: Status::InternalServerError` (기본 Catcher 또는 커스텀 구현)

## 7. 구현 가이드라인

* **TDD**: `tests/` 디렉토리에 통합 테스트를 우선 작성하고 이를 통과하도록 기능을 구현. 필요시 단위 테스트 추가.
* **환경 변수**: `.env` 파일을 통해 설정을 관리하고, `config.rs` 에서 로드하여 `Config` 구조체로 관리. Rocket 상태 관리를 통해 핸들러에서 접근.
...
* **비동기**: Rocket 핸들러 및 관련 I/O 작업은 `async`/`await` 사용. `main` 함수는 `#[rocket::main]` 또는 `#[tokio::main]` 사용.